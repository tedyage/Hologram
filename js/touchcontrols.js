...
var touchstart = function(event){
  ...
  //两个手指触碰，缩放物体
    ...
    for(var i = 0;i < scaleTypeArr.length; i++){
      scaleTypeArr[i] = model_arr[i].scale.x;
    }
  }
};
...
var touchmove = function(event){
  ...
  //两个手指触碰，缩放物体
    ...
    model_arr.forEach(function(model,i){
      if(model===null)
        return;
      var currentScale = currentDistance/touchDistance*scaleTypeArr[i];
      scaleModels(model,currentScale);
      scaleTypeArr[i]=currentScale;
    });
